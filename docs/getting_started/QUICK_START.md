...
The backend will start on `http://localhost:4500` with API documentation available at `http://localhost:4500/docs`.
...
The frontend will be available at `http://localhost:4501`.

## 🎯 Your First Workflow

### Step 1: Access the Platform
1. Open your browser to `http://localhost:4500`
...
curl http://localhost:4500/workflows/enhanced/status
```
...
curl -X POST http://localhost:4500/workflows/enhanced/intelligence/generate \
...
3. **Test the Integration**:
   ...
   curl http://localhost:4500/api/v1/slack/health
   ```
...
response = requests.post(
    "http://localhost:4500/workflows/enhanced/intelligence/generate",
    json={
