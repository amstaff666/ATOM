...
export default async function handler(req, res) {
  ...
  const { provider = 'ollama', messages, apiKey: bodyKey, context, preferEnv } = req.body || {}
  ...
  if (!apiKey && providerNeedsApiKey(provider)) {
    ...
    return res.status(400).json({
      error: { message: hint ? `API võti puudub. Lisa Seadetes või sea dev serveris ${hint}.` : 'API võti puudub.' },
    })
  }
