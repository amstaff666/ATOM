...
function pickNavStyles(override) {
  if (!override) return {}
  const s = {}
  for (const k of STYLE_KEYS) {
    if (override[k] != null) s[k] = override[k]
  }
  return s
}

const links = [
  { to: '/influencers', id: 'nav.link.influencers', label: 'Minu Oll' },
  { to: '/inspiration', id: 'nav.link.inspiration', label: 'Inspiration' },
  { to: '/brand-deals', id: 'nav.link.brand-deals', label: 'Brand Deals' },
]

export default function Nav() {
  const { pathname } = useLocation()
  const { isDark, toggle } = useTheme()
  const { getElementOverride } = usePageEditor()
    ...
    <nav className="nav-root" style={{
      ...
      <NavLink to="/" style={{ marginRight: 'auto', display: 'flex', alignItems: 'center', gap: 9, textDecoration: 'none' }}>
        ...
        <Editable
          as="span"
          id="nav.brand"
          className="nav-brand-label"
          defaultText="Adrenaline Studio"
          style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        {links.map(l => {
          const linkOv = pickNavStyles(getElementOverride(l.id))
          return (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8 }}>
        {(() => {
          const createOv = pickNavStyles(getElementOverride('nav.create'))
          return (
            <NavLink to="/create" style={({ isActive }) => ({
              padding: '7px 16px', borderRadius: 980,
              background: createOv.background || createOv.backgroundColor || (isActive ? (dark ? 'rgba(255,255,255,0.14)' : '#1D1D1F') : dark ? 'rgba(255,255,255,0.12)' : 'linear-gradient(135deg,#EC4899,#8B5CF6)'),
              color: createOv.color || '#fff', fontSize: 13, fontWeight: 700,
              textDecoration: 'none', letterSpacing: '-0.1px',
              boxShadow: dark ? 'none' : '0 2px 8px rgba(139,92,246,0.3)',
              transition: 'all 0.15s',
              ...createOv,
            })}>
              <Editable id="nav.create" defaultText="+ Create" style={{ display: 'inline', color: 'inherit' }} />
            </NavLink>
          )
        })()}

        <button
          data-nav-id="nav.theme"
        ...
        <NavLink to="/settings" data-nav-id="nav.settings" title="Settings" style={({ isActive }) => ({
