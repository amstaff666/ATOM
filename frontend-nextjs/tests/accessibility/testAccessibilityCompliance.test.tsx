...
describe('WCAG 2.1 AA Compliance Tests', () => {
  describe('Home Page Accessibility', () => {
    it('should have no WCAG violations on home page', async () => {
      ...
      await axeCheckViolations(container, 'Home Page', defaultAxeOptions);
    });

    it('should have no critical or serious violations on home page', async () => {
      const { container } = render(<HomePage />);
      await axeCheckCritical(container, 'Home Page');
    });

    it('should expose free edition messaging on home page', async () => {
      render(<HomePage />);
      expect(screen.getByText(/free edition/i)).toBeInTheDocument();
    });

    it('should have accessible get started button on home page', async () => {
      render(<HomePage />);
      const ctaButton = screen.getByRole('button', { name: /get started with automation/i });
