...
/**
 * Normalize FastAPI / Atom API error payloads to a display string.
 */
export function extractApiErrorMessage(
  ...
  if (typeof payload === 'string') {
    const nested = payload.match(/'message':\s*'([^']+)'/);
    if (nested?.[1]) {
      return nested[1];
    }
  ...
  if (typeof payload === 'object') {
    ...
    if (typeof record.message === 'string' && record.message.trim()) {
      return extractApiErrorMessage(record.message, fallback);
    }
    ...
    if (record.detail !== undefined) {
      const nested = extractApiErrorMessage(record.detail, '');
      if (nested) {
        return nested;
      }
    }
...
/**
 * Coerce any value (including API error objects) to a safe React display string.
 */
export function coerceToDisplayString(
  value: unknown,
  fallback = 'An unexpected error occurred. Please try again.',
): string {
  if (value == null) {
  ...
  const extracted = extractApiErrorMessage(value, '');
  if (extracted) {
    return extracted;
  }
...
/**
 * Resolve a user-visible message from an axios/API error.
 */
export function getApiErrorMessage(
  ...
  if (typeof error?.userMessage === 'string' && error.userMessage.trim()) {
    return coerceToDisplayString(error.userMessage, fallback);
  }

  const data = error?.response?.data;
  if (data !== undefined) {
    const fromDetail = extractApiErrorMessage(
    ...
    if (fromDetail) {
      return coerceToDisplayString(fromDetail, fallback);
    }
  }

  const friendly = getUserFriendlyErrorMessage(error);
  return coerceToDisplayString(friendly || fallback, fallback);
}
