...
import { clearAuthTokens, getAuthToken } from "./auth-token";

// Base API configuration (browser uses same-origin proxy)
...
apiClient.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    if (status && status >= 400 && status < 500) {
      if (status === 401 && !originalRequest.__retriedWithoutAuth) {
        clearAuthTokens();
