...
def _normalize_requested_user_id(requested_user_id: Optional[str], current_user: User) -> str:
    """Map legacy default_user requests to the authenticated dev user in local mode."""
    allow_dev_temp_users = os.getenv("ALLOW_DEV_TEMP_USERS", "false").lower() == "true"
    if allow_dev_temp_users and requested_user_id == "default_user":
...
async def rename_session(
    ...
    **Security**: Requires authentication and verifies user owns the session
    """
    try:
        requested_user_id = _normalize_requested_user_id(request.user_id, current_user)
        ...
        if current_user.id != requested_user_id:
            logger.warning(
                ...
                f"session {session_id} belonging to {requested_user_id}"
            )
            raise HTTPException(
                status_code=403,
...
async def get_session_details(
    ...
    **Security**: Requires authentication and verifies user owns the session
    """
    try:
        requested_user_id = _normalize_requested_user_id(user_id, current_user)
        # Security: Verify authenticated user matches the requested user_id (prevents IDOR)
        if current_user.id != requested_user_id:
            logger.warning(
    ...
    try:
        ...
        if current_user.id != requested_user_id:
            logger.warning(
                ...
                f"memory for session {session_id} belonging to {requested_user_id}"
            )
            raise HTTPException(
                status_code=403,
                detail="Access denied: You can only access your own chat sessions"
    ...
    try:
        ...
        if current_user.id != requested_user_id:
            logger.warning(
                ...
                f"history for session {session_id} belonging to {requested_user_id}"
            )
            raise HTTPException(
                status_code=403,
                detail="Access denied: You can only access your own chat sessions"
    ...
    try:
        ...
        if current_user.id != requested_user_id:
            logger.warning(
                ...
                f"sessions belonging to {requested_user_id}"
            )
            raise HTTPException(
                status_code=403,
                detail="Access denied: You can only access your own chat sessions"
