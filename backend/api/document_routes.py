...
async def _resolve_embedding(lancedb_handler, content: str) -> Optional[List[float]]:
    """Generate embedding via async service, with deterministic dev fallback."""
    embedding = await lancedb_handler.async_embed_text(content)
    if embedding is not None:
        return _embedding_to_list(embedding)

    try:
        from core.lancedb_handler import MockEmbedder
        mock = MockEmbedder(384)
        mock_vec = mock.encode(content, convert_to_numpy=True)
        logger.warning("Using mock embedding fallback for document storage")
        return _embedding_to_list(mock_vec)
...
async def _store_document_record(
    ...
    """Persist a document record without triggering optional async side effects."""
    lancedb_handler._ensure_db()
    if lancedb_handler.db is None:
        logger.error("LanceDB is not initialized; cannot store document")
        return False

    vector = await _resolve_embedding(lancedb_handler, content)
    if vector is None:
        return False
    ...
    record = {
        "id": doc_id,
        "user_id": user_id,
        "workspace_id": workspace_id or lancedb_handler.workspace_id,
        "text": content,
        "source": source,
    ...
    table = lancedb_handler.get_table(table_name)
    if table is None:
        lancedb_handler.db.create_table(table_name, data=[record])
    else:
        table.add([record])
...
async def ingest_document(
    ...
    try:
        ...
        success = await _store_document_record(
            lancedb_handler,
            "documents",
            doc_id,
            content,
            f"api:{doc_id}",
            metadata,
            str(current_user.id) if current_user else "default_user",
            ws_id,
        )
