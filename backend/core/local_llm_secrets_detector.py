...
If no secrets found, return: {{"has_secrets": false, "confidence": 0.95, "secrets": []}}"""
    ...
    def __init__(
        ...
        env_provider = os.getenv("LOCAL_LLM_PROVIDER", "").strip().lower()
        if env_provider and provider == LocalLLMProvider.OLLAMA:
            try:
                provider = LocalLLMProvider(env_provider)
            except ValueError:
                logger.warning(f"Unsupported LOCAL_LLM_PROVIDER: {env_provider}")

        env_model = os.getenv("LOCAL_LLM_MODEL") or os.getenv("OLLAMA_MODEL")
        env_ollama_host = os.getenv("OLLAMA_BASE_URL") or os.getenv("OLLAMA_HOST")

        resolved_host = ollama_host
        if env_ollama_host and ollama_host == "http://localhost:11434":
            resolved_host = env_ollama_host

        self.provider = provider
        self.model = model or env_model
        self.ollama_host = _normalize_ollama_host(resolved_host)
    ...
    async def _init_ollama(self) -> bool:
        ...
        try:
            ...
            self._available_models = _extract_ollama_model_names(models_response)
...
def is_ollama_available() -> bool:
    ...
    try:
        import ollama
        host = _normalize_ollama_host(os.getenv("OLLAMA_BASE_URL", "http://localhost:11434"))
        client = ollama.Client(host=host)
        models = client.list()
        return len(_extract_ollama_model_names(models)) > 0
    except Exception as e:
