...
class LanceDBHandler:
    ...
    def _ensure_db(self):
        ...
        if self.db is None and self._lancedb_runtime_available():
            logger.info("Lazy loading LanceDB connection...")
            self._initialize_db()

    @staticmethod
    def _lancedb_runtime_available() -> bool:
        """Check at call time so installs after server start are detected."""
        try:
            import importlib.util
    ...
    def _schedule_background_coroutine(self, coro, description: str) -> bool:
        """Schedule a coroutine only when a running event loop is available."""
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
            logger.info(f"Skipping {description}: no running event loop")
            coro.close()
            return False
    ...
    def add_document(self, table_name: str, text: str, source: str = "",
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if extract_knowledge and not skip_ai_triggers:
                        ...
                        if settings.is_extraction_enabled():
                            ...
                            self._schedule_background_coroutine(
                                ...
                                f"knowledge extraction for {doc_id}",
                            )
                        else:
                    ...
                    if not skip_ai_triggers:
                        try:
                            ...
                            self._schedule_background_coroutine(
                                get_orchestrator().trigger_event("document_uploaded", {
                                    "text": text,
                                    "doc_id": doc_id,
                                    "source": source,
                                    "metadata": metadata
                                }),
                                f"workflow event for {doc_id}",
                            )
                    ...
                    if not skip_ai_triggers:
                        try:
                            ...
                            self._schedule_background_coroutine(
                                ...
                                f"AI trigger coordination for {doc_id}",
                            )
                ...
                except Exception as post_insert_err:
                    logger.warning(f"Document stored but post-insert hooks failed for {doc_id}: {post_insert_err}")
...
def get_lancedb_handler(workspace_id: Optional[str] = None) -> 'LanceDBHandler':
    ...
    handler = _workspace_handlers.get(ws_id)
    if handler is None:
        ...
        handler = LanceDBHandler(db_path=ws_path, workspace_id=ws_id)
        _workspace_handlers[ws_id] = handler
    else:
        handler._ensure_db()
        if handler.db is None and handler._lancedb_runtime_available():
            base_uri = os.getenv("LANCEDB_URI_BASE", "./data/atom_memory")
            ws_path = os.path.join(base_uri, ws_id)
            logger.info(f"Recreating LanceDBHandler for workspace: {ws_id} at {ws_path}")
            handler = LanceDBHandler(db_path=ws_path, workspace_id=ws_id)
            _workspace_handlers[ws_id] = handler
