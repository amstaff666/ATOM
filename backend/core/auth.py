...
def _is_dev_auth_bypass_enabled() -> bool:
    """Allow unauthenticated dev access unless explicitly disabled."""
    explicit = os.getenv("ALLOW_DEV_TEMP_USERS")
...
def _get_dev_fallback_user(db: Session) -> Optional[User]:
    """Return the bootstrap admin user for local development when auth is disabled."""
    if not _is_dev_auth_bypass_enabled():
        return None

    user = db.query(User).filter(User.email == "admin@example.com").first()
    if user:
        logger.warning("⚠️ Using local development auth fallback user")
    return user
...
def _dev_fallback_or_raise(db: Session, credentials_exception: HTTPException) -> User:
    """Use bootstrap admin in local dev instead of returning 401."""
    fallback_user = _get_dev_fallback_user(db)
    if fallback_user:
...
async def get_current_user(
    ...
    try:
        return await _resolve_current_user(request, token, db, credentials_exception)
    except HTTPException as exc:
        if exc.status_code == status.HTTP_401_UNAUTHORIZED:
            return _dev_fallback_or_raise(db, credentials_exception)
        raise
...
async def _resolve_current_user(
    ...
    # Local dev: unauthenticated browser requests (no Bearer) use bootstrap admin
    if not has_bearer and _is_dev_auth_bypass_enabled():
        return _dev_fallback_or_raise(db, credentials_exception)
